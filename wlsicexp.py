...
def fit(time, trajectories, guess, min_method='nm'):
    ...
    p_opt, p_sigma, chi2_min = minimize(time, y_mean, R, C, guess, min_method, bounds=None)
    # p_opt, p_sigma, chi2_min = differential_evolution(time, y_mean, R, C, guess, min_method)

    return (p_opt, p_sigma, chi2_min)


# differential_evolution(func, bounds, args=(), strategy='best1bin', maxiter=1000, popsize=15, tol=0.01, mutation=(0.5, 1), recombination=0.7, seed=None, callback=None, disp=False, polish=True, init='latinhypercube', atol=0, updating='immediate', workers=1)
